...
class AdministratorsController extends AppController {
    ...
    public function check_pass() {
        if (!empty($this->data)) {
            ...
            if ($data['Administrator']['pass_old'] == '' || $data['Administrator']['pass_new'] == '') {
                ...
                if ($check['Administrator']['password'] != md5($data['Administrator']['pass_old'])) {
                    echo "<script>alert('" . json_encode('Mật khẩu cũ không đúng! Vui lòng thực hiện lại!') . "');</script>";
                    echo "<script>history.back();</script>";
                } else {
                    $data['Administrator']['password'] = md5($data['Administrator']['pass_new']);
